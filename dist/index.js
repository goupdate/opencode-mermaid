...
var MAX_DATA_URL = 1e5;
var MermaidInlinePlugin = async () => {
  return {
    // Inject system prompt so the AI knows it can use mermaid blocks
    "experimental.chat.system.transform": async (_input, output) => {
      output.system.push(
        "You can include Mermaid diagrams in your responses using ```mermaid code blocks. The plugin will automatically render them as inline SVG images. Supported types: flowchart (graph TD/LR/RL/BT), sequenceDiagram, classDiagram, stateDiagram-v2, erDiagram. NOT supported: pie, gantt, mindmap, timeline, gitGraph \u2014 use flowchart instead for those. Example:\n```mermaid\ngraph TD\n  A[Client] --> B[Server]\n  B --> C[(Database)]\n```"
      );
    },
